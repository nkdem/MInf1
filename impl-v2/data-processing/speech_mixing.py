...
def create_directory_structure(base_dir):
    ...
    environments = ['InTraffic', 'InVehicle','Music', 'QuietIndoors', 'ReverberantEnvironment', 'WindTurbulence']
...
def get_split_counts():
    ...
    splits = {
        'InTraffic': {'background': 530 * 2, 'speech': 470 * 2},
        'InVehicle': {'background': 584 * 2, 'speech': 510 * 2},
        'Music': {'background': 1496 * 2, 'speech': 1495 * 2},
        'QuietIndoors': {'background': 525 * 2, 'speech': 426 * 2},
        'ReverberantEnvironment': {'background': 315 * 2, 'speech': 692 * 2},
        'WindTurbulence': {'background': 595 * 2, 'speech': 439 * 2}
    }
    return splits
...
def split_background_files(background_files, bg_target_count, speech_target_count):
    """Split background files according to target counts, keeping L/R pairs and—if possible—a complete cut_id together.
    
    The naming scheme for snippets is:
    
        <ENV_ID>_<REC_ID>_<CUT_ID>_<SNIP_ID>_<TRACKNAME>_<SAMPLERATE>.wav
    
    L and R files (e.g. containing '_L_' and '_R_') for the same snippet (as given by the base filename)
    are kept together. In addition, this version tries to avoid having files from the same CUT_ID 
    appear in both the background and speech sets. (If a complete cut group is too large to fit entirely
    into one set’s remaining quota, it may be split, with a warning.)
    """
    # Ensure target counts are even (since we need L/R pairs)
    ...
    # Group files by their base name (which should correspond to a unique snippet, i.e. one L file and one R file)
    file_pairs = {}
    ...
    # Verify that each base is a valid L/R pair
    ...
    # Group base keys by CUT_ID.
    # We assume that split by underscore gives:
    # [<ENV_ID>, <REC_ID>, <CUT_ID>, <SNIP_ID>, ...]
    cut_groups = {}
    for base in file_pairs.keys():
        parts = base.split('_')
        # Ensure there are at least 3 parts; otherwise, raise an error.
        if len(parts) < 3:
            raise ValueError(f"Base '{base}' does not have enough parts to extract a CUT_ID")
        cut_id = parts[2]
        if cut_id not in cut_groups:
            cut_groups[cut_id] = []
        cut_groups[cut_id].append(base)

    # Convert cut_groups to a list of (cut, list_of_bases) and shuffle it
    group_list = list(cut_groups.items())
    random.shuffle(group_list)

    selected_bg_bases = []
    selected_speech_bases = []

    needed_bg = bg_pair_count
    needed_speech = speech_pair_count

    # Greedy assignment by cut group
    for cut, bases in group_list:
        group_size = len(bases)
        # If the whole group can fit into both sets, assign it to the set which needs more pairs.
        if group_size <= needed_bg and group_size <= needed_speech:
            if needed_bg >= needed_speech:
                selected_bg_bases.extend(bases)
                needed_bg -= group_size
            else:
                selected_speech_bases.extend(bases)
                needed_speech -= group_size
        # If it can fit entirely into one set, do so.
        elif group_size <= needed_bg:
            selected_bg_bases.extend(bases)
            needed_bg -= group_size
        elif group_size <= needed_speech:
            selected_speech_bases.extend(bases)
            needed_speech -= group_size
        else:
            # The group is larger than the remaining need in both sets.
            # In this case, we must split the cut group—which is not ideal.
            print(f"Warning: Splitting CUT_ID group {cut} with {group_size} pairs between sets")
            if needed_bg >= needed_speech and needed_bg > 0:
                # Assign as many as possible to background.
                chosen_bg = random.sample(bases, needed_bg)
                selected_bg_bases.extend(chosen_bg)
                # The remainder goes to speech, if it fits.
                remaining = [b for b in bases if b not in chosen_bg]
                if len(remaining) <= needed_speech:
                    selected_speech_bases.extend(remaining)
                    needed_speech -= len(remaining)
                else:
                    chosen_speech = random.sample(remaining, needed_speech)
                    selected_speech_bases.extend(chosen_speech)
                    needed_speech = 0
                needed_bg = 0
            elif needed_speech > 0:
                # Assign as many as possible to speech.
                chosen_speech = random.sample(bases, needed_speech)
                selected_speech_bases.extend(chosen_speech)
                remaining = [b for b in bases if b not in chosen_speech]
                if len(remaining) <= needed_bg:
                    selected_bg_bases.extend(remaining)
                    needed_bg -= len(remaining)
                else:
                    chosen_bg = random.sample(remaining, needed_bg)
                    selected_bg_bases.extend(chosen_bg)
                    needed_bg = 0
                needed_speech = 0

        if needed_bg == 0 and needed_speech == 0:
            break

    if needed_bg != 0 or needed_speech != 0:
        raise ValueError(f"Not enough pairs to meet target counts. Still need {needed_bg} background pairs and {needed_speech} speech pairs.")

    # Reconstruct complete file lists from the base keys
    background_set = []
    for base in selected_bg_bases:
        background_set.extend(file_pairs[base])
    speech_set = []
    for base in selected_speech_bases:
        speech_set.extend(file_pairs[base])

    # Final assertions on set sizes
    ...
    # Double-check L/R pairs haven’t been broken
    ...
    print(f"Background pairs: {len(selected_bg_bases)}, total files: {len(background_set)}")
    print(f"Speech pairs: {len(selected_speech_bases)}, total files: {len(speech_set)}")
