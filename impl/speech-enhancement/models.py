...
# Took from https://www.isca-archive.org/interspeech_2018/tan18_interspeech.html
class CRNN(nn.Module):
    """
    Input: [batch size, channels=1, T, F]
    Output: [batch size, T, F]
    # NOTE: Librosa operates in [F, T]! So care is needed when using the model.
    """
    def __init__(self):
        ...
        # LSTM
        self.LSTM1 = nn.LSTM(input_size=1024, hidden_size=1024, num_layers=2, batch_first=True) # VOICEBANK
        # self.LSTM1 = nn.LSTM(input_size=7680, hidden_size=7680, num_layers=2, batch_first=True) # HEARDS
