...
\abstract{
   ...
   to investigate the effectiveness of machine learning algorithms in prioritising speech in noisy environments. 
   At its forefront, the project aims to develop machine learning models that could be 
   used in hearing aids to prioritise speech in noisy environments. So the project 
   has to be mindful of the models computational complexity. And so, with every model
   we present, we not only present the performance of the model, but also 
   argue its feasibility in a hearing aid by considering metrics such as Floating Point Operations Per Second (FLOPS) and 
   the number of parameters in the model.
   
   From the market we have found that there is a trend towards incorporating deep neural networks (DNNs) 
   into hearing aids. From a particular manufacturer, we could deduce that they use a combination of Acoustic Scene Analysis (ASA) and 
   Speech Enhancement techniques. As such, this project will aim to mimic this behaviour by 
   ...
   We conduct the experiments on a novel dataset called HEAR-DS where 
   we evaluate the baseline Convolutional Neural Network (CNN) ASA model presented by the authors of the dataset. 
   We demonstrate techniques that improve both the training process and generalization capability of the CNN model through data augmentation and 
   the usage of a different optimiser. 
   
   We then take this dataset further by demonstrating a proof-of-concept of a speech enhancement model that could be used as a state-dependent model.
   For Speech Enhancement, we present a shallow CNN model that works in the frequency-time domain and evaluate its performance by using artificial scores such as STOI and PESQ.



   
   % We then compare the performance of the CNN model to a Recurrent Neural Network (RNN) model, specifically 
   % the Long Short-Term Memory (LSTM). The results show that the LSTM model outperforms the CNN model.  << MAYBE IF I HAVE TIME



   %  We then take inspiration 
...
in prioritising speech in noisy environments. The idea is to first 
...
used to detect speech, however, we will instead 
mix the samples with speech and label them as so. 
...
to train a speech enhancement model. Additionally, we will 
be looking at techniques that improve the training process and generalization 
...
we will cover each in separate sections.

\subsection{Acoustic Scene Analysis (ASA)}
Acoustic Scene Analysis (ASA) refers to the task of classifying an environment based on an audio stream. Mathematically, ASA can be formalised as follows:
...
The focus of this study is the HEAR-DS dataset introduced by \citet{Huwel2020HearDS}. This dataset was developed in response to the inadequacies of existing acoustic scene classification databases for HA processing. A more detailed description of the dataset is provided in Chapter \ref{sec:datasets}.
...
Speech enhancement aims to improve the perceptual quality of a speech signal that 
...
will be focusing on the Transform-Domain Filtering approach. Part two of this dissertation will be 
investigating how ASA and Speech Enhancement can be combined to form a state-dependent model.
