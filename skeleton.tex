...
\subsection{Speech Enhancement}
\heards has not been used for SE before, and so we will be 
exploring the feasibility of using the dataset for SE.
We will be taking heavy inspiration from the paper by \citet{tan18_interspeech}
who discussed a convolutional encoder-decoder (CED) model. While the 
paper has 'real-time speech enhancement' in its title, there 
is no evaluation on the performance of the model in terms of 
real-time inference or the feasibility of the model in a HA device.
This was unfortunately a common theme in the SE literature, and so we will be 
and various papers that introduce SE models and a lot of them address thta 
SE models applications include HAs, and yet there is little 
discussion on the feasibility of the models in a HA device. 
This could be due to the little amout of research 
on the capabilities of HA devices, and so we wish 
to contribute to this area of research since 
there is some new insight to be had from reading 
...
\subsection{Evaluation}
The speech enhancement model's performance is evaluated using PESQ and STOI,
