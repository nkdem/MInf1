...
%    \includegraphics[width=0.9\textwidth]{state-dependent-model.png}
%    \caption{The vision we have for the state-dependent model.}
%    \label{fig:state-dependent-model}
% \end{figure}
\chapter{Experimental Setup}
This chapter will be dedicated to explaining the hypotheses we aim to validate in this part of the dissertation.
Firstly, we will outline the hypotheses, and then explain the datasets we will be using to validate the hypotheses.
\section{Hypotheses}
...
\section{Datasets}
\label{sec:datasets}
% Big table of all the statistics of the datasets
% DATASET | ENVIRONMENTS (if applicable) | number of samples | number of speakers (if applicable) | max duration of samples | min duration of samples | mean duration of samples | std duration of samples
...
% by having environments that contain speech and those that do not. As it stands, the dataset does not contain any 
% samples that do not contain speech, and we will follow a similar approach to the paper to create these mixed environments.
% While the dataset could have recorded the environments with and without speech, it did not, and for good reason.
% It was argued that the ability to have full control of the SNR in which the speech is mixed was an important consideration. 
% There is one deviation we will take from the paper's approach, at least for this part of the dissertation, and that is 
% to not incorporate a head-related transfer function (HRTF) in the creation of the mixed environments. This is 
% due to constraints in the time we had and the lack of digestable libraries for me to implement this. In any case,
% we will be using the same approach to create the mixed environments as the paper. The paper's approach was to use 
% the CHiME 3 dataset (see Section \ref{sec:chime3} for more details about the dataset) to mix the speech signal with the background noise.
% They ensured that each mixed environment contained at least 7.5 seconds of speech in a 10s snippet. The selection of speakers and the 
% splitting of the speakers is unclear from the paper, so we will be doing the following:
% Since we will have a training and test set, we will be splitting the speakers into 2 groups. We will be using the training bucket of speakers 
% for the training set and the test bucket of speakers for the test set. As can be seen from Table \ref{tab:chime3-stats}, the CHiME 3 dataset contains 
% 4 speakers, and conveniently enough an equal split of gender, hence we ensured for generalisability that each set contained a male and female speaker. 
