...
\subsection{Acoustic Scene Analysis (ASA)}
Acoustic Scene Analysis is the process of classifying the scene (environment) from an audio stream. 
Mathematically, ASA can be defined as follows:

Given an audio signal $x \in \mathbb{R}^{T}$, where $T$ is the number of samples in the signal, 
and a set of $N$ environments $\mathcal{E} = \{e_1, e_2, \ldots, e_N\}$. Then ASA is the process 
of finding a mapping $f: \mathbb{R}^{T} \rightarrow \mathcal{E}$ that maps the audio signal to one of the 
environments in $\mathcal{E}$. $f$ for the purposes of this project is a neural network. So to 
find the mapping, we need to train the neural network on a dataset of audio signals and their corresponding environments. As such, we need
...
Speech enhancement is the process of maximising the perceptual quality of a speech signal, and 
more specifically, the process of removing 'noise' from a speech signal, this could be background noise, such 
as a 'wind' environment, or it could be other types of noise, such as 'babble' noise from multiple speakers.
Mathematically, speech enhancement can be defined as follows:
Given a noisy speech signal $x \in \mathbb{R}^{T}$, and a clean speech signal $y \in \mathbb{R}^{T}$, where $T$ is the number of samples in the signal,
then the goal of speech enhancement is to find a mapping $f: \mathbb{R}^{T} \rightarrow \mathbb{R}^{T}$ that approximates $y$ given $x$ i.e. 
$f(x) \approx y$. $f$ is once again a neural network for the purposes of this project. And once again, we need a dataset to train the model on.
TODO: Talk about baseline model
