...
This dissertation explores the intersection of Machine Learning, Automatic Speech Recognition (ASR), and Hearing Aids (HA) through the lens of Acoustic Scene Analysis (ASA) and Speech Enhancement (SE).
The primary objective is to develop machine learning models capable of prioritising speech in noisy environments while remaining computationally efficient for implementation in hearing aids.
...
Evidence from state-of-the-art manufacturers suggests that a combination of ASA and SE techniques is employed, informing the approach adopted in this project.
Initially, the project classifies the acoustic environment using a robust machine learning model; subsequently, heuristic speech enhancement techniques are applied to selectively enhance speech in environments that demand it.


Experiments are conducted using a novel dataset known as HEAR-DS.
As a baseline, the commonly used Convolutional Neural Network (CNN) ASA model introduced by the dataset’s authors is utilised.
The study demonstrates improvements in both the training procedure and the generalisation capability of the baseline CNN through the implementation of data augmentation strategies and the adoption of an alternative optimiser.


Furthermore, a proof-of-concept is presented for a state-dependent speech enhancement model based on a shallow CNN operating in the frequency-time domain.
The performance of this model is rigorously evaluated using standard artificial intelligibility and quality metrics, including the Short-Time Objective Intelligibility (STOI) and Perceptual Evaluation of Speech Quality (PESQ) scores.
...
\abstract{
   ...
   The idea is to chain the ASA and SE models together to form a state-dependent model, in essence,
...
The researchers investigated the effect of different model parameters, where the first 
