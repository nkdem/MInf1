...
\end{table}
\footnotetext{{Assumes a 7,700M FLOPs capable device and that the following formula is used: 
\scriptsize{
\[
% \text{Inference Time (ms)} = \text{FLOP(s) Count}\div{\text{FLOPs}} \times 1000
\text{Inference Time (ms)} = \frac{\text{FLOP(s) Count}}{\text{FLOPs}} \times 1000
\]
}
}
}
...
\begin{table}[h]
   ...
   \begin{tabular}{lccc}
      ...
      Model & FLOP(s) Count & Parameters & Theoretical Inference Time\footnotemark \\
      \midrule
      CED (10s) & 25.244G & 17.579M & 3.2784415584 seconds \\ % 3.2784415584 seconds
...
\footnotetext{{As in ASA feasibility analysis, assumes a 7,700M FLOPs capable device.}}
