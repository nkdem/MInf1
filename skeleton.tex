...
\section{Feasibiltiy} 
% Before we do a cursory analysis of the speech enhancement task, we will summarise 
% the results of the experiments we have done so far and end on a discussion of the 
% feasibility of the task from a HA perspective.
% We started off with a baseline implementation of the ASA that aimed to mimic the paper 
% by \citet{Huwel2020HearDS} as closely as possible. We were getting a higher accuracy than the paper,
% but it's most likely due to the difference in dataset used for speech mixing. We then investigated 
% whether we can improve the generalisation of the model by utilising data augmentation at the speech 
% mixing proces. The results were not as promising as we hoped from a performance perspective, but the 
% training time was substantially reduced. Afterwards, we investigated the claim 
% by \citet{Huwel2020HearDS} that the Adam optimiser is sensitive to the initialisation,
% but applying the Adam optimiser to the same setup as the data augmentation approach, 
% we were able to get comparable approach to that of using the SGD optimiser.
% And to validate that our implementation is correct, we were able to get comparable 
% performance to the paper by \citet{schindler_multi-temporal_2018}.
...
% table with the model (net-8, net-20, net-32) and the model size, FLOPS and number of parameters
\begin{table}[h]
   \centering
   \begin{tabular}{lccc}
      \toprule
...
We extended the data preparation to also normalise the spectrograms to have zero mean and unit variance so 
that the MSE loss is stable. The following experiments all are done using the Adam optimiser with a learning rate of 0.001 
and early stopping was applied when the loss does not improve after 5 epochs and with a batch size of 32.
...
\chapter{Conclusions}
\section{Discussion}
% Hypotheses' pertaining to ASA achieved 
% SE task is more difficult, and more work is needed. Given that the FLOP count is low, we can look into much 
% deeper models 
% 
TODO
\section{Future Work}
